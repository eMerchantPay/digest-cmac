...
describe 'CMAC' do
  ...
  context 'valid cipher block size' do

    let(:key) { ['2b7e151628aed2a6abf7158809cf4f3c'].pack('H*') }
    let(:cipher) { OpenSSL::Cipher.new('aes-128-cbc') }
    let(:cmac) { Digest::CMAC.new(cipher, key) }
  ...
  context 'invalid cipher block size' do

    let(:invalid_block_cipher) { OpenSSL::Cipher.new('aes-128-gcm') }

    it 'raises invalid block size exception' do
      expect {
        Digest::CMAC.new(invalid_block_cipher, ['2b7e151628aed2a6abf7158809cf4f3c'].pack('H*'))
      }.to raise_error(RuntimeError, "Cipher block size must be #{Digest::CMAC::BLOCK_SIZE}")
    end
